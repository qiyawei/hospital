    ...
    <div class="row-fluid">
        <div class="span12">
            ...
            <span class="title">
             ...
             <a href="/disease/list.do"> 病种列表</a>  /  新增疾病
            </span>
                </div>
                <div class="box-body form">
                    <form action="/disease/update.do" method="post">
                        <input type="hidden" name="disease.id" value="${disease.id}" >
                        <label>疾病名称</label>
                        <input type="text" name="disease.diseaseName" value="${disease.diseaseName}">
                        <label>所属科室</label>
                        <select id="ks" name="disease.dept.id">
                            <option value="">请选择科室</option>
                            <c:forEach items="${deptList}" var="dept">
                                <option value="${dept.id}" ${disease.dept.id == dept.id?'selected':'' }>${dept.deptName}</option>
                            </c:forEach>>
                        </select>
