            ...
            <span class="title">
              ...
              <a href="patient-list.html"> 患者列表</a>  /  新增患者
                ...
                <div class="box-body form">
                    <form action="/paitent/update.do" method="post">
                        <input type="hidden" name="id" value="${patient.id}">
                        <input type="hidden" name="paitent.id" value="${paitent.id}">
                        <label>姓名</label>
                        <input type="text" name="paitent.name"value="${paitent.name}">
                        <label>性别</label>
                        ...
                        <label>身份证号</label>
                        <input type="text" name="paitent.card" value="${paitent.card}">
                        <label>年龄</label>
                        <input type="text" name="paitent.age" value="${paitent.age}" >
                        <label>电话</label>
                        <input type="text" name="paitent.tel" value="${paitent.tel}">
                        <label>医保类型</label>
                        <select name="paitent.insurance.id" id="yb">
                            <option value="">请选择医保类型</option>
                            <c:forEach items="${insuranceList}" var="insur">
                                <option value="${insur.id}" ${insur.id == paitent.insurance.id?'selected':''}>${insur.insurName}</option>
                            </c:forEach>
                        </select>
                        <label>住址</label>
                        <input type="text" name="paitent.address" value="${paitent.address}">
                        <label>过敏史</label>
                        <textarea  class="editor1" style="height:50px" name="paitent.allergy">${paitent.allergy}</textarea>
                        <label>备注</label>
                        <textarea  class="editor2" name="paitent.text">${paitent.text}</textarea>
